...
def _derive_key(password: str, salt: bytes) -> bytes:
    ...
    # 第一轮：密码与 salt 逐字节 XOR 混合，扩展到 32 字节
    seed = bytes(
...
class MCPKWriter:
    ...
    def add_file(
        ...
        if created_at is None:
            # Windows: ctime 是创建时间; Linux: ctime 是 inode 变更时间
            ct = stat.st_ctime
    ...
    def finalize(self):
        ...
        for original_idx, entry in sorted_entries:
            ...
            if encrypt_blobs:
                ...
                # stored_size 更新为磁盘上的实际字节数（含 salt）
